...
+ (NSString *)stringWithDescending:(NSDate *)date
{
    NSDate *current = [NSDate date];
    double since  = [current timeIntervalSinceDate:date];
    if (since <= 60) {
        return @"1 分钟前";
    } else if (since > 60 && since <= 600) {
        return [NSString stringWithFormat:@"%0.f 分钟前", floor(since / 10)];
    } else if (since > 600 && since <= 3600 * 24) {
        return [self stringFromDate:date dateFormat:@"hh:mm"];
    } else {
        
        if ([date isSameYearAsDate:current]) {
            return [self stringFromDate:date dateFormat:@"MM-dd hh:mm"];
        } else {
